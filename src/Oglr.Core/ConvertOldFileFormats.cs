...
using Oglr.Core.Legacy;
using Oglr.InGame;
using StructureMap;
using CustomProperty = Oglr.InGame.CustomProperty;
using Level = Oglr.Core.Legacy.Level;
...
namespace Oglr.Core
{
	public class ConvertOldFileFormats
	{
		...
		{
			Level oldLevel=Level.FromFile( path ) ;
		...
		{
			var editor =
                (ItemEditor)Activator.CreateInstance(getType(@"Oglr.Plugins.TextureItemEditor, Oglr.Plugins"));
